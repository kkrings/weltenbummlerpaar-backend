...
export class DiaryEntriesDBServiceMock extends DiaryEntriesDBServiceBase {
  ...
  async findMany (): Promise<DiaryEntry[]> {
    return [...this.diaryEntriesCollection]
  }

  async findOne (diaryEntryId: string): Promise<DiaryEntry> {
    return await throwOnNull(diaryEntryId, async () => this.diaryEntriesCollection
  ...
  async updateOne (
    ...
    const images = updateDiaryEntryDto.images?.map(
      imageId => diaryEntry.images.filter(image => image._id.equals(imageId))[0]
    )
  ...
  async removeOne (diaryEntryId: string): Promise<DiaryEntry> {
    return await throwOnNull(diaryEntryId, async () => {
      ...
      return index > -1 ? this.diaryEntriesCollection.splice(index, 1)[0] : null
    })
  }
